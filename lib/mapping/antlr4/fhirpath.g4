...
//version without precedence and left-recursion expression: term (righthand)*; righthand: op term |
// '.' function; term: '(' expression ')' | fpconst | predicate; op: LOGIC | COMP | '*' | '/' | '+'
// | '-' | '|' | '&';
...
predicate: ('$context' | '$resource' | '$parent' | item) (
		'.' item
	)*;
...
fpconst: STRING | '-'? NUMBER | BOOL | CONST;
...
fragment ESC:
	'\\' (["'\\/bfnrt] | UNICODE); // allow \", \', \\, \/, \b, etc. and \uXXX
